...
class EEGDataset(Dataset):
    def __init__(self, spect, lyapunovs, labels):
        self.spect = torch.tensor(spect, dtype=torch.float32)
        self.lyapunovs = torch.tensor(lyapunovs, dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        return self.spect[idx], self.lyapunovs[idx], self.labels[idx]
...
def prepare_data(use_zg=True):
    ...
    with open('/mnt/s3-data2/amiftakhova/OCD/lyapunovs_zg.pkl', 'rb') as f:
        lyapunovs = pickle.load(f)
    lyapunovs = [np.concatenate(x) for x in lyapunovs]
    return spectrograms, labels, lyapunovs

def main():
    spectrograms, labels, lyapunovs = prepare_data(use_zg=True)
    ...
    train_dataset = EEGDataset(np.concatenate([spectrograms[i] for i in train_ids]), np.concatenate([lyapunovs[i].reshape(-1, 19) for i in train_ids]), np.concatenate([class_labels[i] for i in train_ids]))
    test_dataset = EEGDataset(np.concatenate([spectrograms[i] for i in test_ids]), np.concatenate([lyapunovs[i].reshape(-1, 19) for i in test_ids]), np.concatenate([class_labels[i] for i in test_ids]))
    ...
    for j, lr in enumerate(lrs):
        ...
        for epoch in tqdm(range(num_epochs), desc='Training'):
            model.train()
            losses = []
            for input_spect, input_lyapunov, targets in train_loader:
                input_spect = input_spect.to(device)
                input_lyapunov = input_lyapunov.to(device)
                targets = targets.to(device)
                optimizer.zero_grad()
                outputs = model(input_spect, input_lyapunov)
            ...
            losses = []
            accs = []
            for input_spect, input_lyapunov, targets in test_loader:
                with torch.no_grad():
                    input_spect = input_spect.to(device)
                    input_lyapunov = input_lyapunov.to(device)
                    targets = targets.to(device)
            ...
            plt.ylabel('Loss')
            plt.legend()
            plt.savefig(f'lyap_loss_zg_{j}.png')

        X = [spectrograms[i] for i in test_ids]
        L = [lyapunovs[i].reshape(-1, 19) for i in test_ids]
        ...
        with open('lyap_metrics_zg.json', 'w') as f:
            json.dump(metrics, f)

        subject_preds_majority = np.array(subject_preds_majority)
        accuracy = np.mean(subject_preds_majority == subject_true)
        print(f'modified, Majority Accuracy: {accuracy}')
        # torch.save(model, '/mnt/s3-data2/amiftakhova/eeg_cnn/eeg_cnn_zg_lyap_44.pth')
